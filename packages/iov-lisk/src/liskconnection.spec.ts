  ...
  describe("searchTx", () => {
    ...
    it("can search transactions by address", async () => {
      ...
      // by non-existing address
      {
        ...
        const results = await connection.searchTx({ address: unusedAddress });
        expect(results.length).toEqual(0);
      }
      ...
      // by recipient address (from lisk/init.sh)
      {
        ...
        const results = await connection.searchTx({ address: searchAddress });
    ...
    it("can search transactions by address and minHeight/maxHeight", async () => {
      ...
      // minHeight = 2
      {
        const results = await connection.searchTx({ address: searchAddress, minHeight: 2 });
        expect(results.length).toBeGreaterThanOrEqual(1);
        for (const result of results) {
          expect(result.height).toBeGreaterThanOrEqual(2);
          const transaction = result.transaction;
      ...
      // maxHeight = 100
      {
        const results = await connection.searchTx({ address: searchAddress, maxHeight: 100 });
        expect(results.length).toBeGreaterThanOrEqual(1);
        for (const result of results) {
