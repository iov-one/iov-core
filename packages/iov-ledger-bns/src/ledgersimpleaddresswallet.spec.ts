...
describe("LedgerSimpleAddressWallet", () => {
  ...
  it("can deserialize", () => {
    {
      // empty
      const wallet = new LedgerSimpleAddressWallet('{ "formatVersion": 1, "identities": [] }' as WalletSerializationString);
      expect(wallet).toBeTruthy();
      expect(wallet.getIdentities().length).toEqual(0);
    }
    ...
    {
      // one element
      const serialized = '{ "formatVersion": 1, "identities": [{"localIdentity": { "pubkey": { "algo": "ed25519", "data": "aabbccdd" }, "label": "foo" }, "simpleAddressIndex": 7}] }' as WalletSerializationString;
    ...
    {
      // two elements
      const serialized = '{ "formatVersion": 1, "identities": [{"localIdentity": { "pubkey": { "algo": "ed25519", "data": "aabbccdd" }, "label": "foo" }, "simpleAddressIndex": 7}, {"localIdentity": { "pubkey": { "algo": "ed25519", "data": "ddccbbaa" }, "label": "bar" }, "simpleAddressIndex": 23}] }' as WalletSerializationString;
      const wallet = new LedgerSimpleAddressWallet(serialized);
      expect(wallet).toBeTruthy();
  ...
  it("throws for unsupported format version", () => {
    const data = '{ "formatVersion": 123, "identities": [] }' as WalletSerializationString;
  ...
  it("can be cloned", () => {
    const oneIdentitySerialization = '{ "formatVersion": 1, "identities": [{"localIdentity": { "pubkey": { "algo": "ed25519", "data": "aabbccdd" }, "label": "foo" }, "simpleAddressIndex": 7}] }' as WalletSerializationString;
