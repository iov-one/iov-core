...
describe("Governor", () => {
  ...
  describe("getVote", () => {
    it("can get a vote for a given proposal", async () => {
      pendingWithoutBnsd();
      const options = await getGovernorOptions();
      const governor = new Governor(options);
      const { connection, identity, profile } = options;
      ...
      const votePre = await governor.getVote(proposalId);
      expect(votePre).toBeNull();
      ...
      const vote = await governor.getVote(proposalId);
      expect(vote).toEqual(VoteOption.Yes);
