...
describe("Governor", () => {
  ...
  describe("buildCreateProposalTx", () => {
    ...
    it("works for TreasurySend", async () => {
      pendingWithoutBnsd();
      const options = await getGovernorOptions();
      const cleanTreasuryAddress = Bech32.encode("tiov", Random.getBytes(20)) as Address;
      const governor = new Governor({
        ...options,
        treasuryAddress: cleanTreasuryAddress,
      });
      const { connection, identity, profile } = options;
      ...
      const sendTx = await connection.withDefaultFee({
        ...
        creator: identity,
        sender: bnsCodec.identityToAddress(identity),
        recipient: cleanTreasuryAddress,
        amount: {
          quantity: "999000000000",
          fractionalDigits: 9,
          tokenTicker: "CASH" as TokenTicker,
      ...
      const tx = await governor.buildCreateProposalTx({
        ...
        description: "Proposal to send from treasury",
        startTime: new ReadonlyDate(1562164525898),
        electionRuleId: 1,
        recipients: [
          {
